...
trading_instrument_idx = 66   # bitcoin binance = 48, etheruem binance = 66
ask1_idx = 0
bid1_idx = 5
trading_fees = 0.0001
...
class TradingEnv(gym.Env):
  def __init__(self):
    self.action_space = np.arange(-100, 100)
  ...
  def reset(self):
    ...
    # current state
    return np.concatenate((self.x.reshape(-1), [self.share/200], [self.balance/1000000])).reshape((1, -1))
  ...
  def step(self, action):
    ...
    next_state = np.concatenate(
        (self.x.reshape(-1), [self.share/200], [self.balance/1000000])).reshape((1, -1))
...
class Agent:
    ...
    def get_optimal_action(self, state):
        ...
        if np.any(np.isnan(q_values[0])):
          ...
          interval = np.arange(
              np.max([-int(share), -100]), np.min([int(balance/5000), 100]))
...
def create_agent():
    ...
    agent.q_network = keras.models.load_model(
        "/persistent/experiments/q_model_eth.h5")
...
def trading_action(agent, x, share, balance,total):
    ...
    state = np.concatenate(
        (x.reshape(-1), [share/200], [balance/1000000])).reshape((1, -1))
